<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Jean de la croix ISHIMIRWE - Portfolio</title>
  <link rel="stylesheet" href="style.css">
</head>
<body>
  <div class="container">
    <div class="profile">
      <img src="Jean_de_la_croix.jpg" alt="Jean de la croix ISHIMIRWE" class="profile-pic">
      <h1>Jean de la croix ISHIMIRWE</h1>
      <p class="subtitle">Student at Adventist University of Central Africa</p>
      <p class="study">Networks and Communication Systems</p>
      <div class="buttons">
        <a href="mailto:ishimirwejean21@gmail.com" class="btn">Email</a>
        <a href="tel:+250786007413" class="btn">Call</a>
        <a href="https://github.com/ISHIMIRWEJ" target="_blank" class="btn">GitHub</a>
      </div>
    </div>

    <div class="section">
      <h2>About Me</h2>
      <p>
        I am passionate about networking and communication systems, aiming to design
        and implement efficient, secure, and scalable network infrastructures. My goal
        is to contribute to technological advancements and provide innovative solutions
        that empower communities.
      </p>
    </div>

    <div class="section">
      <h2>Skills</h2>
      <ul>
        <li>Network Configuration</li>
        <li>Linux/Unix Systems</li>
        <li>Routing & Switching</li>
        <li>Cybersecurity Basics</li>
        <li>DNS & Web Servers</li>
        <li>Virtualization</li>
      </ul>
    </div>

    <div class="section">
      <h2>Projects</h2>
      <ul>
        <li>Network-Attached Storage (NAS) Deployment</li>
        <li>Suricata IDS for Security Monitoring</li>
        <li>Web Hosting with Apache2 & Nginx</li>
        <li>Secure SSH with 2FA & Firewall Rules</li>
        <li>Dynamic Portfolio with Docker & Traefik</li>
      </ul>
    </div>
  </div>
</body>
</html>
